...
public class UpdateUserServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String sql ="update user SET userFirstName=?, userLastName=?, userEmail=?, userPassword=? WHERE userId=?";
		
		try (Connection connection = db.getConnection();
				PreparedStatement preparedStatement = connection.prepareStatement(sql)){
			
			preparedStatement.setInt(5, sendToMysql.getUserId());
			...
			System.out.println(result > 0 ? "User updated successfully. result is:" +result : "Failed to update user. result is:" +result);
			...
			if (result > 0) {
				...
				System.out.println("User updated successfully. result is:" +result);
				...
				System.out.println("Failed to update user. result is:" +result);
			}
